...
- (Опционально) [LibreOffice](https://www.libreoffice.org/) — `brew install --cask
  libreoffice` — чтобы .pptx/.docx открывались сразу как PDF-превью в браузере, а не
  скачивались. Без него всё работает, просто такие файлы будут скачиваться.
...
- Браузер не умеет показывать .pptx/.docx сам по себе — локальный дашборд конвертирует
  такие файлы в PDF через LibreOffice при синке (если он установлен) и открывает именно
  PDF-версию. Без LibreOffice такие файлы просто скачиваются. Веб-версия через Claude
  Artifact на файлы диска доступа не имеет — там всегда ссылка на страницу файла в Canvas.
